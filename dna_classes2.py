...
class Gene(object):
    def __init__(self, dna=None, exon_regions=None):
        ...
        if (
            ...
            raise TypeError(
                "dna=%s %s is not string or (urlbase,filename) " "tuple" % (dna, type(dna))
            )
        ...
        if er is None:
            ...
            if (
                ...
                raise TypeError(
                    ...
                    "or (urlbase,filename) tuple" % (er, type(er))
                )
    ...
    def __str__(self):
        ...
        s = "Gene: " + self._dna[:6] + "..." + self._dna[-6:] + ", length=%d" % len(self._dna)
        if self._exons is not None:
...
def test_lactase_gene():
    ...
    lactase_gene = ProteinCodingGene((urlbase, lactase_gene_file), (urlbase, lactase_exon_file))
